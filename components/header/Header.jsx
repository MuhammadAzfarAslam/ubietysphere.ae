    ...
    return (
      <li className="relative group lg:static">
        ...
        <ul
          ...
          } bg-white shadow-lg lg:absolute mt-2 py-2 rounded-md w-40 z-50`}
        >
    ...
    <header>
      <div className="block bg-primary py-2 d-none d-sm-block text-white fw-bold w-full max-w-screen">
        <div className="max-w-7xl container mx-auto">
          <div className="row flex items-center justify-between gx-4">
            <div className="col-auto d-none d-lg-block text-xs">
              1010 Avenue, New York, NY 10018 US.
            </div>
            <div className="col-auto">
              <button className="hover:underline text-white px-8 rounded-md text-xs">Login</button>
            </div>
          </div>
        </div>
      </div>
      <nav className="block w-full max-w-screen px-4 py-3 mx-auto bg-white bg-opacity-90 sticky top-3 shadow lg:px-8 backdrop-blur-lg backdrop-saturate-150 z-[9999]">
        <div className="container flex flex-wrap items-center justify-between max-w-7xl mx-auto text-slate-800">
          <Link href="/" className="mr-4 block cursor-pointer py-1.5 text-red-600 font-bold text-2xl">
          ...
          <div className={`fixed top-0 left-0 min-h-screen w-64 bg-slate-100 shadow-lg transform transition-transform duration-300 ease-in-out ${isMobileMenuOpen ? "translate-x-0" : "-translate-x-full"} lg:hidden z-50`}>
            ...
            <ul className="flex flex-col h-full gap-4 p-4">
              {menu.map((item, index) => (
                <MenuItem key={index} item={item} onClick={() => setIsMobileMenuOpen(false)} />
              ))}
              <li className="mt-4">

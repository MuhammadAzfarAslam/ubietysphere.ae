...
const createSchema = (isLoggedIn) => yup.object({
  ...
  doctor: yup.string().nullable(), // optional if provided via props
...
const BookingForm = ({ doctorName = null }) => {
  ...
  const onSubmit = async (data) => {
    ...
    const res = await postData(`booking-form`, formData);
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="mt-4 space-y-4">
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
        ...
        {/* Doctor Dropdown (hidden if doctorName provided) */}
        {!doctorName && (
          <div>
            <label className="block text-sm font-medium text-gray-700">
              Choose Doctor*
      ...
      <div className="flex items-center gap-5">
        <FormButton additionalClass={'w-full'} disabled={isSubmitting}>Book An Appointment</FormButton>
        {isSubmitting && (
          <svg
            aria-hidden="true"
